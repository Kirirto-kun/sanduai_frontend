  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-3xl border border-white/60 px-6 py-6 shadow-md sm:px-8">
        <h2 className="text-xl font-semibold text-slate-900">{t.article.form.title}</h2>
        <form className="mt-4 grid gap-4 sm:grid-cols-2" onSubmit={onGenerate}>
            ...
            <div className="mt-2 space-y-2">
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  value="scientific"
                  checked={form.genre === "scientific"}
                  onChange={(e) => setForm((f) => ({ ...f, genre: e.target.value as any }))}
                  className="h-4 w-4 text-[color:var(--primary)] focus:ring-[color:var(--primary)]"
                />
                <span className="text-sm text-slate-700">{t.article.genres.scientific}</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  value="publicistic"
                  checked={form.genre === "publicistic"}
                  onChange={(e) => setForm((f) => ({ ...f, genre: e.target.value as any }))}
                  className="h-4 w-4 text-[color:var(--primary)] focus:ring-[color:var(--primary)]"
                />
                <span className="text-sm text-slate-700">{t.article.genres.publicistic}</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  value="custom"
                  checked={form.genre === "custom"}
                  onChange={(e) => setForm((f) => ({ ...f, genre: e.target.value as any }))}
                  className="h-4 w-4 text-[color:var(--primary)] focus:ring-[color:var(--primary)]"
                />
                <span className="text-sm text-slate-700">{t.article.genres.custom}</span>
              </label>
            </div>
          </div>
          ...
          <div className="sm:col-span-2 flex justify-end">
            ...
            >
              {loading ? t.article.loading : t.article.form.generate}
            </button>
          </div>
        </form>
      </div>
